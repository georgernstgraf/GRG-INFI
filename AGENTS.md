...
- `lehrplan/`: Standard-Lehrplan-Layout mit Zweig-Ebene (Skill `lehrplan`, Retrofit 2026-09-10): Zweig-Ordner `infi-hwii/` (HWII, Anlage 1.24) mit `LEHRPLAN.md` (offizieller ①-Extrakt), `RIS.md` (Rechtsstand/Schichten-Vergleich), `HWII_INFI.pdf` (② Schuladaption), Klassen-Extrakten `2HWII/`–`5HWII/` je mit `<KLASSE>.lehrplan.md` und — im aktuellen Ordner `3HWII/` — Klassen-Drehscheibe (`README.md`), plus `kompetenzmodule/` (Didaktische Steckbriefe, `README.md` = Matrix, `kmN.md` pro KM). Skelett `infi-hwit/` (Anlage 1.28, HWIT; Extraktion ausstehend, primäre Verwaltung im Fachgruppen-Repo). `METADATA.md` auf `lehrplan/`-Ebene. Unterrichts-Ebene am Repo-Root: `unterricht/HWII-INFI/` mit `jgN-einheiten.md` und `jg3-semesterplan-{ws,ss}.md` (PMM-Stil, 13 UE + 2 PLF, SJ 2026/27, KM5/KM6, SWP-Verbund-Doku). Klassenordner groß geschrieben (mögliche Parallelklassen).
